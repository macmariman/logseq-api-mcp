...
_page_result = {"id": 1, "uuid": "new-uuid", "originalName": "New Page",
                 "format": "markdown", "journal?": False}
...
class TestCreatePage:
    ...
    async def test_shows_page_uuid(self):
        page_result = {"id": 1, "uuid": "abc-123", "originalName": "My Page",
                       "format": "markdown", "journal?": False}
    ...
    async def test_properties_count_shown(self):
        client = FakeLogseqClient({"create_page": _page_result})
        result = await _run(client, _cfg, "P", properties={"status": "active", "priority": "high"})
    ...
    async def test_frontmatter_merged_with_properties(self):
        ...
        await _run(capturing, _cfg, "New Page", content=md, properties={"priority": "high"})
