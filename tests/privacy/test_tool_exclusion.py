...
class TestQueryExclusion:
    async def test_excluded_page_absent_from_query_results(self):
        ...
        items = [
            {"originalName": "Secret Page", "name": "secret page", "uuid": "p1", "journal?": False},
            {"originalName": "Public Page", "name": "public page", "uuid": "p2", "journal?": False},
        ]
        client = FakeLogseqClient({"query_dsl": items, "get_all_pages": [_PUBLIC, _SECRET]})
...
class TestGetAllPageContentExclusion:
    async def test_excluded_target_page_returns_access_denied(self):
        ...
        assert "access" in result[0].text.lower() or "denied" in result[0].text.lower() or "excluded" in result[0].text.lower()
...
class TestGetPageBacklinksExclusion:
    async def test_excluded_source_page_absent_from_backlinks(self):
        ...
        linked_refs = [
            [{"id": hash("Secret Page"), "name": "secret page", "originalName": "Secret Page"},
             {"uuid": "b1", "content": "ref"}],
            [{"id": hash("Public Page"), "name": "public page", "originalName": "Public Page"},
             {"uuid": "b2", "content": "ref"}],
        ]
        client = FakeLogseqClient({
