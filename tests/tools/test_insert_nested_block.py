...
_parent = {"id": 1, "uuid": "parent-uuid", "content": "parent block",
...
class TestInsertNestedBlock:
    async def test_insert_as_child_default(self):
        ...
        result = await _run(client, _cfg, "parent-uuid", "child content")
    ...
    async def test_insert_as_sibling_when_sibling_true(self):
        ...
        result = await _run(client, _cfg, "parent-uuid", "sibling content", sibling=True)
        insert_calls = [c for c in client.calls if c[0] == "insert_block"]
    ...
    async def test_insert_with_properties(self):
        ...
        result = await _run(client, _cfg, "parent-uuid", "content", properties={"status": "todo"})
