...
async def test_vector_search_returns_results_when_available(vector_config):
    mock_results = [
        {"page": "Alpha", "chunk_index": 0, "text": "Block one Block two", "_distance": 0.1},
    ...
    with patch("src.vector.search.lancedb") as mock_lancedb, \
...
async def test_vector_search_returns_disabled_message_when_not_configured():
    ...
    assert len(result) == 1
    assert "not enabled" in result[0].text.lower() or "disabled" in result[0].text.lower()


async def test_vector_search_returns_error_when_lancedb_missing(vector_config):
    with patch("src.vector.search.lancedb", None), \
         patch("src.vector.search.load_vector_config", return_value=vector_config):
        result = await vector_search("hello")

    assert len(result) == 1
    assert "not installed" in result[0].text.lower() or "unavailable" in result[0].text.lower()
...
async def test_vector_search_handles_open_table_failure(vector_config):
    ...
    assert len(result) == 1
    assert "❌" in result[0].text or "no results" in result[0].text.lower() or "not found" in result[0].text.lower()


async def test_vector_search_respects_limit(vector_config):
    mock_results = [
        {"page": f"Page{i}", "chunk_index": 0, "text": f"text {i}", "_distance": float(i) * 0.1}
        for i in range(5)
    ]
    mock_table = MagicMock()
    mock_table.search.return_value.limit.return_value.to_list.return_value = mock_results
