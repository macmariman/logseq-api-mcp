...
The PNG is a transient preview for the agent's eyes only — Logseq renders the
.excalidraw natively and does not need it. Always pass --output to a scratch
path OUTSIDE the Logseq graph so the PNG never lands next to the source file in
<graph>/draws/.

Usage:
    cd .claude/skills/logseq/references/excalidraw
    uv run python render_excalidraw.py <path-to-file.excalidraw> --output /tmp/logseq-excalidraw-preview/<file>.png [--scale 2] [--width 1920]
...
def render(
    ...
    try:
        ...
        print("Run: cd .claude/skills/logseq/references/excalidraw && uv sync && uv run playwright install chromium", file=sys.stderr)
        sys.exit(1)
    ...
    with sync_playwright() as p:
        try:
            ...
            if "Executable doesn't exist" in str(e) or "browserType.launch" in str(e):
                ...
                print("Run: cd .claude/skills/logseq/references/excalidraw && uv run playwright install chromium", file=sys.stderr)
                sys.exit(1)
            raise
